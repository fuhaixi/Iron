...
void GPU::render_object(Object& obj){
...
void GPU::render(Rmesh const& rm, int mode_offset){
    glBindVertexArray(rm.gid);

    glPolygonMode(GL_FRONT_AND_BACK, GL_POINT + mode_offset);

    glDrawElements(GL_TRIANGLES, rm.size, GL_UNSIGNED_INT, NULL);
}

Rmesh GPU::meshes_new(Mesh const& mesh){
    GLuint VAO;
    GLuint VBO;
    GLuint EBO;
...
Rshader GPU::shaders_new(const char* vertex, const char* fragment){
    uint id = IRON_GL::create_shader_program(vertex, fragment);
    Rshader rs;
    rs.gid = id;
...
Rshader GPU::shaders_new_from_file(const char* vshader_file_name, const char* fshader_file_name){
    ...
    uint id = IRON_GL::create_shader_program(vbuffer.str().c_str(), fbuffer.str().c_str(), fshader_file_name);
    Rshader rs;
    rs.gid = id;
    shaders.push_back(rs);
...
Rtexture GPU::texture_new(const Img& img){
    uint id = IRON_GL::create_texture(img.width, img.height, img.data);
    Rtexture rt;
    rt.gid = id;
...
Rtexture GPU::cubemap_new(Img* faces){
    Rtexture rt;
...
Rtexture GPU::texture_new(unsigned int width, unsigned int height,int format, bool repeat, bool linear_filter){
...
Rtexture GPU::rgb16f_tex_new(unsigned int width, unsigned int height, bool repeat, bool linear_filter){
    unsigned int textureColorbuffer;
    glGenTextures(1, &textureColorbuffer);
    glBindTexture(GL_TEXTURE_2D, textureColorbuffer);
...
void GPU::renderbuff_new(uint width, uint height){
    unsigned int rbo;
...
Rtexture GPU::depth_texture_new(unsigned int width, unsigned int height){
...
Rframebuff GPU::depth_framebuffer_new(Rtexture depth_map){
    Rframebuff ret;
...
Rframebuff GPU::color_frambuff_new(Rtexture color_tex0){
...
Rframebuff GPU::framebuff_new(Rtexture color_tex0, Rtexture color_tex1, bool use_depth){
    Rframebuff ret;
    unsigned int fbo;
    glGenFramebuffers(1, &fbo);
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
...
void GPU::use_frame(Rframebuff rf){
...
void GPU::bind_tex_at(Rtexture& rt , unsigned int pos){
    
    glActiveTexture(GL_TEXTURE0 + pos);
    glBindTexture(GL_TEXTURE_2D, rt.gid);
}

void GPU::bind_cubemap_at(Rtexture& rt , unsigned int pos){
    glActiveTexture(GL_TEXTURE0 + pos);
    glBindTexture(GL_TEXTURE_CUBE_MAP, rt.gid);
                                                            
}

void GPU::use(Rshader shader){
    glUseProgram(shader.gid);
    shader_id = shader.gid;
}


void GPU::shader_set(const std::string& name, bool value){
    glUniform1i(glGetUniformLocation(shader_id, name.c_str()), (int)value); 
    
}


void GPU::shader_set(const std::string &name, int value) 
{ 
    glUniform1i(glGetUniformLocation(shader_id, name.c_str()), value); 

}
// ------------------------------------------------------------------------
void GPU::shader_set(const std::string &name, float value) 
{ 
    glUniform1f(glGetUniformLocation(shader_id, name.c_str()), value); 
}

void GPU::shader_set(const std::string &name, mat4 value) 
{
    
    glUniformMatrix4fv(glGetUniformLocation(shader_id, name.c_str()), 1, GL_FALSE, value.data());
}

void GPU::shader_set(const std::string &name, vec3 vec) {
    glUniform3fv(glGetUniformLocation(shader_id, name.c_str()), 1, vec.data());

}

void GPU::shader_set_viewer(Viewer viewer){
